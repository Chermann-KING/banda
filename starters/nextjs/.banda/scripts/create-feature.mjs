...
// 2. Renommer les fichiers dont le nom contient {{FeatureName}}
function renameInDir(dir) {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const full = join(dir, entry.name);
    if (entry.isDirectory()) {
      renameInDir(full);
    } else if (entry.name.includes('{{FeatureName}}')) {
      const newName = entry.name.replaceAll('{{FeatureName}}', featurePascal);
...
function replaceInDir(dir) {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      replaceInDir(full);
    } else if (entry.isFile() && /\.(ts|tsx|md)$/.test(entry.name)) {
