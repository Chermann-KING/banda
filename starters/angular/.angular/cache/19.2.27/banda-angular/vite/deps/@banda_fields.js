...
// ../../packages/banda-fields/dist/sanitize.js
var DANGEROUS_PATTERN = /[<>\u0000-\u001F\u007F]/g;
function stripDangerous(raw) {
  return raw.replace(DANGEROUS_PATTERN, "");
}
var NOT_NAME_PATTERN = /[^\p{L}\s'’-]/gu;
function sanitizeName(raw) {
  return stripDangerous(raw).replace(NOT_NAME_PATTERN, "");
}
var NOT_ALPHANUMERIC_PATTERN = /[^\p{L}\p{N}\s'’-]/gu;
function sanitizeAlphanumeric(raw) {
...
function formatTitleCase(value) {
  return value.toLocaleLowerCase().replace(new RegExp("(^|[\\s'’-])(\\p{L})", "gu"), (_match, separator, letter) => {
...
function formatLowercase(value) {
  return value.toLocaleLowerCase();
}
function formatThousands(value) {
  const [integer = "", decimals] = value.split(",");
  const grouped = integer.replace(/\B(?=(\d{3})+(?!\d))/g, " ");
  return decimals === void 0 ? grouped : `${grouped},${decimals}`;
}
...
// ../../packages/banda-fields/dist/password.js
var PASSWORD_MIN_LENGTH = 8;
function evaluatePasswordStrength(password, minLength = PASSWORD_MIN_LENGTH) {
  const criteria = {
    minLength: password.length >= minLength,
    lowercase: new RegExp("\\p{Ll}", "u").test(password),
    uppercase: new RegExp("\\p{Lu}", "u").test(password),
    digit: /\d/.test(password),
    symbol: /[^\p{L}\p{N}\s]/u.test(password)
  };
  const satisfied = Object.values(criteria).filter(Boolean).length;
  const score = password.length === 0 ? 0 : Math.max(1, Math.min(4, satisfied - 1));
  return {
...
// ../../packages/banda-fields/dist/phone/countries.js
var PHONE_COUNTRIES = [{
  iso: "GA",
  name: "Gabon",
  flag: "🇬🇦",
...
var DEFAULT_PHONE_COUNTRY = PHONE_COUNTRIES[0];

// ../../packages/banda-fields/dist/phone/phone.js
function getPhoneCountry(iso) {
  return PHONE_COUNTRIES.find((country) => country.iso === iso) ?? DEFAULT_PHONE_COUNTRY;
}
function phoneTotalDigits(country) {
...
function sanitizePhone(raw, country) {
...
function isPhoneComplete(digits, country) {
...
// ../../packages/banda-fields/dist/specs/field-spec.js
function defineFieldSpec(spec) {
  return spec;
}
...
// ../../packages/banda-fields/dist/specs/specs.js
var NAME_PATTERN = /^[\p{L}][\p{L}\s'’-]*$/u;
var ISO_DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;
var isValidIsoDate = (value) => !Number.isNaN((/* @__PURE__ */ new Date(`${value}T00:00:00`)).getTime());
var firstNameSpec = defineFieldSpec({
  id: "first-name",
  type: "text",
  ...
  schema: external_exports.string().min(2, "Le prénom doit contenir au moins 2 caractères.").max(50, "Le prénom ne peut pas dépasser 50 caractères.").regex(NAME_PATTERN, "Caractères non autorisés dans un prénom.")
});
var lastNameSpec = defineFieldSpec({
  id: "last-name",
  type: "text",
  ...
  finalize: collapseWhitespace,
  schema: external_exports.string().min(2, "Le nom doit contenir au moins 2 caractères.").max(50, "Le nom ne peut pas dépasser 50 caractères.").regex(NAME_PATTERN, "Caractères non autorisés dans un nom.")
});
var emailSpec = defineFieldSpec({
  id: "email",
  type: "email",
  inputMode: "email",
  autoComplete: "email",
  ...
  schema: external_exports.string().min(1, "L'email est requis.").max(254, "L'email est trop long.").email("Adresse email invalide.")
});
var passwordSpec = defineFieldSpec({
  id: "password",
  type: "password",
  autoComplete: "new-password",
  hint: `Au moins ${PASSWORD_MIN_LENGTH} caractères, avec majuscule, minuscule et chiffre`,
  maxLength: 128,
  schema: external_exports.string().min(PASSWORD_MIN_LENGTH, `Au moins ${PASSWORD_MIN_LENGTH} caractères.`).regex(new RegExp("\\p{Ll}", "u"), "Au moins une minuscule.").regex(new RegExp("\\p{Lu}", "u"), "Au moins une majuscule.").regex(/\d/, "Au moins un chiffre.")
});
var citySpec = defineFieldSpec({
  id: "city",
  type: "text",
  ...
  format: formatTitleCase,
  finalize: collapseWhitespace,
  schema: external_exports.string().min(1, "La ville est requise.").max(80, "La ville ne peut pas dépasser 80 caractères.")
});
var streetSpec = defineFieldSpec({
  id: "street",
  type: "text",
  ...
  sanitize: sanitizeAlphanumeric,
  finalize: collapseWhitespace,
  schema: external_exports.string().max(120, "La rue ne peut pas dépasser 120 caractères.")
});
var houseNumberSpec = defineFieldSpec({
  id: "house-number",
  type: "text",
  inputMode: "numeric",
  placeholder: "Ex : 12B",
  ...
  format: formatUppercase,
  schema: external_exports.string().max(10, "10 caractères maximum.")
});
var dateSpec = defineFieldSpec({
  id: "date",
  type: "date",
  schema: external_exports.string().min(1, "La date est requise.").regex(ISO_DATE_PATTERN, "Format de date invalide.").refine(isValidIsoDate, "Cette date n’existe pas.")
});
var birthDateSpec = defineFieldSpec({
  id: "birth-date",
  type: "date",
  autoComplete: "bday",
  schema: external_exports.string().min(1, "La date de naissance est requise.").regex(ISO_DATE_PATTERN, "Format de date invalide.").refine(isValidIsoDate, "Cette date n’existe pas.").refine((value) => /* @__PURE__ */ new Date(`${value}T00:00:00`) <= /* @__PURE__ */ new Date(), "La date de naissance ne peut pas être dans le futur.").refine((value) => {
    const age = (Date.now() - (/* @__PURE__ */ new Date(`${value}T00:00:00`)).getTime()) / 315576e5;
    return age <= 120;
  }, "Vérifiez l’année de naissance.")
});
var amountSpec = defineFieldSpec({
  id: "amount",
  type: "text",
  inputMode: "decimal",
  ...
  schema: external_exports.string().min(1, "Le montant est requis.").refine((value) => !Number.isNaN(parseAmount(value)), "Montant invalide.").refine((value) => parseAmount(value) >= 0, "Le montant doit être positif.")
});
var urlSpec = defineFieldSpec({
  id: "url",
  type: "url",
  inputMode: "url",
  autoComplete: "url",
  ...
  schema: external_exports.string().min(1, "L'URL est requise.").url("URL invalide.")
});
var otpSpec = defineFieldSpec({
  id: "otp",
  type: "text",
  inputMode: "numeric",
  ...
  schema: external_exports.string().regex(/^\d{6}$/, "Le code doit contenir exactement 6 chiffres.")
});
var searchSpec = defineFieldSpec({
  id: "search",
  type: "search",
  inputMode: "search",
  ...
  finalize: collapseWhitespace,
  schema: external_exports.string().max(200)
});
function makePhoneSpec(iso) {
  ...
  return defineFieldSpec({
    ...
    schema: external_exports.string().min(1, "Le numéro de téléphone est requis.").refine((value) => isPhoneComplete(value, country), "Numéro incomplet pour ce pays.")
  });
}
var fieldSpecs = {
...
//# sourceMappingURL=@banda_fields.js.map
