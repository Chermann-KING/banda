 ...
 * Module PRIVÉ (non exporté du barrel) : alignement visuel garanti spec —
 * même hauteur, même border, même focus ring pour tous les champs.
...
export function controlClasses({
  ...
  return [BASE, SIZE[size], CONTROL_VARIANT[variant], invalid && ERROR, extra]
    .filter(Boolean)
    .join(' ');
}
